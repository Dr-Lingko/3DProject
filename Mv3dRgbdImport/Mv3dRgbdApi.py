...
def _iter_candidate_dll_dirs():
    # You can override/add a runtime directory with MV3D_RGBD_DLL_DIR.
    env_dirs = os.environ.get("MV3D_RGBD_DLL_DIR", "")
    if env_dirs:
    ...
    yield r"C:\Program Files (x86)\Common Files\MV3D\Runtime\Win64_x64"
    yield r"C:\Program Files (x86)\Common Files\Mv3dRgbdSDK\Runtime\Win64_x64"
    yield r"C:\Program Files\Common Files\MV3D\Runtime\Win64_x64"
    yield r"C:\Program Files\Common Files\Mv3dRgbdSDK\Runtime\Win64_x64"


def _load_mv3d_rgbd_dll():
    dll_name = "Mv3dRgbd.dll"
    searched_dirs = []
    seen = set()

    for d in _iter_candidate_dll_dirs():
    ...
    # Final fallback to PATH / current process loader behavior.
    try:
        return ctypes.WinDLL(dll_name)
    except OSError as exc:
        tips = [
            "Unable to load Mv3dRgbd.dll.",
            "Set MV3D_RGBD_DLL_DIR to your SDK Runtime directory if needed.",
            "Searched directories:",
            *[f"  - {p}" for p in searched_dirs],
        ]
        raise FileNotFoundError("\n".join(tips)) from exc


Mv3dRgbdDll = _load_mv3d_rgbd_dll()
